...
turtles-own [energy location]       ;; both wolves and sheep have energy
...
to setup
  ...
  set-default-shape sheep "hex" ;; "sheep"
  ...
  set-default-shape wolves "hex" ;; "wolf"
  ...
  display-labels
;  set grass count patches with [color = green]
  reset-ticks
end
...
to go
  ...
  if grass? [ ask nodes [ grow-grass ] ]
 set grass count nodes with [color = green]
  tick
  display-labels
end
...
to eat-grass  ;; sheep procedure
 ...
 ;; print "trying to eat"
  let food one-of nodes-here
 ;; if node color = green  [
 if  ([color] of food =  green) [
  show [color] of food
  ;;  print "eating"
  ;; set one-of nodes-here red
    ask food [set color brown]
...
"grass / 6" 1.0 0 -10899396 true "" "if grass? [ plot grass / 6 ]"
